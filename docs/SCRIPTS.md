...
Most commands live in the project root as `.mjs` modules and are exposed via `npm run <name>`; a few helpers such as `dashboard:build` and `test:*` wrap other validated entrypoints.
...
| `npm run dashboard:build` | workspace-local Go wrapper   | Build `dashboard/career-dashboard`       |
| `npm run test:quick`      | `test-all.mjs --quick`       | Run the split-app smoke suite            |
| `npm run test:full`       | `test-all.mjs`               | Run the full repository validation suite |
