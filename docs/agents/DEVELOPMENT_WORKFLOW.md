...
| `npm run dashboard:build`  | Build the dashboard with the workspace-local Go toolchain wrapper.    |
| `npm run test:quick`       | Run the repository smoke suite, including split app validation.       |
| `npm run test:full`        | Run the full validation suite, including dashboard compilation.       |
...
7. If `npm run backend:check`, `npm run frontend:install && npm run frontend:build`, or `npm run test:quick` fails for a split-app change, block the migration/cleanup instead of adding more hidden compatibility branches.
8. If you change dashboard code, run `npm run dashboard:build` before you consider the work done.
...
- Use `npm run test:quick` before opening a PR that touches split-app behavior, shared contract docs, or split-app cleanup work.
- Treat failures in `npm run backend:check`, `npm run frontend:install && npm run frontend:build`, or `npm run test:quick` as blocking for contract upgrades, alias retirement, and split-app cleanup.
...
- Use `npm run dashboard:build` for dashboard validation.
